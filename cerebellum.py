...
def main():
    ...
    pygame.display.set_caption("Neuro Flow: Motor/Sensory/Cerebellum/Basal/Thalamus")
    ...
    C = Block(360, 230, 280, 80, "Spinal Cord\nLower Motor Neurons")
    ...
    for (s, e) in [(A, B), (B, C), (C, D), (D, E)]:
        connections.append(Connection((s, "bottom", 0.0), (e, "top", 0.0),
                                      color=chain_color, width=3, sparks=3, spark_speed=0.8, **conn_kwargs))

    # Sensory feedback (E -> F), then F -> G and F -> H
    connections.append(Connection((E, "bottom", 0.0), (F, "top", 0.0),
                                  color=SENSORY_1, width=3, sparks=3, spark_speed=0.8, **conn_kwargs))
    connections.append(Connection((F, "left", 0.0), (G, "right", 0.0),
                                  color=SENSORY_2, width=3, sparks=2, spark_speed=0.7, **conn_kwargs))
    ...
    connections.append(Connection((A, "left", -0.2), (I, "right", -0.2),
                                  color=CEREBELLUM, width=3, sparks=3, spark_speed=0.8, **conn_kwargs))
    ...
    connections.append(Connection((K, "bottom", 0.0), (H, "top", 0.0),
                                  color=BASAL, width=3, sparks=3, spark_speed=0.8, **conn_kwargs))

    # Thalamus hub (H -> A)
    connections.append(Connection((H, "right", 0.0), (A, "right", 0.0),
                                  color=THALAMUS, width=3, sparks=4, spark_speed=0.9, **conn_kwargs))

    # Exception: Olfactory (L -> A)
    connections.append(Connection((L, "left", 0.0), (A, "bottom", 0.3),
                                  color=OLFACT, width=3, sparks=3, spark_speed=0.8, **conn_kwargs))
